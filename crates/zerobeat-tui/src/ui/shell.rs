...
fn render_sidebar(frame: &mut Frame, area: Rect, app: &App, hits: &mut HitMap) {
    ...
    push_nav(&mut lines, &mut targets, app, Route::Home, "Home", None);
    push_nav(&mut lines, &mut targets, app, Route::Search, "Search", None);
    lines.push(Line::raw(""));
    ...
    lines.push(menu_line("Lyrics", app.lyrics().visible, None));
    lines.push(Line::raw(""));
...
fn push_nav(
    ...
    lines.push(menu_line(label, app.route() == route, count));
}

fn menu_line(label: &str, active: bool, count: Option<usize>) -> Line<'static> {
...
fn section(label: &str) -> Line<'static> {
    Line::styled(label.to_owned(), Style::default().fg(theme::TEXT_MUTED))
}
